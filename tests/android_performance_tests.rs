...
fn test_room_extraction_performance() {
    ...
    println!("Room extraction: {:?} for {} elements, {} relationships", 
        elapsed, elements.len(), relationships.len());
...
fn test_hilt_extraction_performance() {
    ...
    println!("Hilt extraction: {:?} for {} elements, {} relationships", 
        elapsed, elements.len(), relationships.len());
...
fn test_manifest_extraction_performance() {
    ...
    println!("Manifest extraction: {:?} for {} elements, {} relationships", 
        elapsed, elements.len(), relationships.len());
    
    assert!(elapsed.as_millis() < 100, "Manifest extraction should be fast");
...
fn test_resource_ref_extraction_performance() {
    ...
    println!("Resource ref extraction: {:?} for {} relationships", 
        elapsed, relationships.len());
    
    assert!(elapsed.as_millis() < 50, "Resource ref extraction should be very fast");
}
...
fn test_batch_extraction_performance() {
    ...
    println!("Batch extraction: {:?} for {} elements, {} relationships across {} files", 
        elapsed, total_elements, total_relationships, files.len());
    
    assert!(elapsed.as_millis() < 500, "Batch extraction should complete quickly");
}
...
fn test_extraction_scalability() {
    ...
    println!("Large file extraction: {:?} for {} bytes, {} elements", 
        elapsed, large_source.len(), elements.len());
