...
use leankg::indexer::{AndroidHiltExtractor, AndroidManifestExtractor, AndroidResourceRefExtractor, AndroidRoomExtractor};
use std::fs;
...
fn test_tv_app_full_extraction() {
    ...
    let app_rels: Vec<_> = manifest_rels.iter()
...
fn test_room_entities_with_relationships() {
    let entity_path = format!("{}/src/main/java/com/tv/app/data/local/entity/ChannelEntity.kt", TV_APP_DIR);
    let entity_source = fs::read_to_string(&entity_path)
        .expect("Entity file not found");
    ...
    assert!(!fk_rels.is_empty() || entities.len() >= 2, 
        "Should have entities with FK relationships or multiple entities");
}

#[test]
fn test_dao_with_queries() {
    let dao_path = format!("{}/src/main/java/com/tv/app/data/local/dao/ChannelDao.kt", TV_APP_DIR);
    let dao_source = fs::read_to_string(&dao_path)
        .expect("DAO file not found");
    ...
    let query_rels: Vec<_> = relationships.iter()
        ...
        .collect();
    assert!(!query_rels.is_empty() || daos.len() > 0, 
...
fn test_hilt_module_providers() {
    ...
    let module_source = fs::read_to_string(&module_path)
        .expect("Hilt module file not found");
    ...
    assert!(modules.len() > 0 || providers.len() > 0 || relationships.len() > 0,
        "Should extract something from Hilt module");
}

#[test]
fn test_repository_with_inject() {
    let repo_path = format!("{}/src/main/java/com/tv/app/data/repository/ChannelRepository.kt", TV_APP_DIR);
    let repo_source = fs::read_to_string(&repo_path)
        .expect("Repository file not found");
    ...
    let inject_rels: Vec<_> = relationships.iter()
    ...
    assert!(repo_source.contains("ChannelRepository"), 
        "Repository should contain class name");
    assert!(repo_source.contains("@Inject") || repo_source.contains("constructor("),
...
fn test_tv_app_structure_completeness() {
    ...
    let expected_files = vec![
        ...
        format!("{}/src/main/java/com/tv/app/data/local/TvDatabase.kt", TV_APP_DIR),
        format!("{}/src/main/java/com/tv/app/data/local/entity/ChannelEntity.kt", TV_APP_DIR),
        format!("{}/src/main/java/com/tv/app/data/local/dao/ChannelDao.kt", TV_APP_DIR),
        format!("{}/src/main/java/com/tv/app/data/remote/PlaylistApi.kt", TV_APP_DIR),
        format!("{}/src/main/java/com/tv/app/data/repository/ChannelRepository.kt", TV_APP_DIR),
        format!("{}/src/main/java/com/tv/app/ui/browse/BrowseFragment.kt", TV_APP_DIR),
        format!("{}/src/main/java/com/tv/app/ui/player/PlayerActivity.kt", TV_APP_DIR),
    ];

    for path in &expected_files {
        assert!(fs::metadata(path).is_ok(), 
            "Expected TV app file should exist: {}", path);
    }
}
...
fn test_cross_file_relationships() {
    ...
    // Database → Entity
    let db_path = format!("{}/src/main/java/com/tv/app/data/local/TvDatabase.kt", TV_APP_DIR);
    let db_source = fs::read_to_string(&db_path)
    ...
    let databases: Vec<_> = elements.iter()
    ...
    let db_entity_rels: Vec<_> = relationships.iter()
...
fn test_resource_references_in_ui() {
    ...
    let fragment_path = format!("{}/src/main/java/com/tv/app/ui/browse/BrowseFragment.kt", TV_APP_DIR);
    let fragment_source = fs::read_to_string(&fragment_path)
        .expect("Fragment file not found");

    // Check for typical Android patterns
    assert!(fragment_source.contains("@AndroidEntryPoint") || 
    ...
    let res_extractor = AndroidResourceRefExtractor::new(fragment_source.as_bytes(), &fragment_path);
